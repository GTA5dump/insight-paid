...
namespace insight {
	void network::n_delete_gun(int player) {
		...
		{
			player_options::get()->n_delete_gun_value[player] = false;
		}
	}
	bool was_n_delete_gun_all_on = false;
	void network::n_delete_gun_all(toggle_feature* feature, bool& value)
	{
		if (!value) {
			if (was_n_delete_gun_all_on) {
				for (int i = 0; i < 32; i++) {
					player_options::get()->n_delete_gun_value[i] = false;
				}
				was_n_delete_gun_all_on = false;
			}
			return;
		}
		was_n_delete_gun_all_on = true;
		for (int i = 0; i < 32; i++) {
			if (tasks::get_all_players_w_include_ex(i))
				player_options::get()->n_delete_gun_value[i] = true;
			else
				player_options::get()->n_delete_gun_value[i] = false;
		}
	}
