...
#pragma pack(pop)
	...
	struct x64Operation {
		...
		uint8_t get_numm_modrm_operands() {
			uint8_t count{};
			for (uint8_t i{}; i != m_maxOperands; ++i)
				if (auto enc = get_opr_encoding(i); enc == M || enc == R)
					++count;
			return count;
		}
	};
	...
	struct x64Instruction {
		...
		std::string toBytecode() {
			...
			if (m_operation->get_numm_modrm_operands() != NULL) {
				uint8_t modrm{};
				modrm |= (0b11 << 6); // direct
		...
		for (auto& op : m_operations) {
			if (op.matches(code)) {
				...
				if (op.m_operandEncoding != ZO) {
					...
					uint8_t opcode = *code;
					uint8_t oprI{};
					uint8_t numModrmOprs = op.get_numm_modrm_operands();
	...
	inline uint8_t get_instruction_length(uint8_t* code) {
		auto start = code;
