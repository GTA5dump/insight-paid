...
private:
	...
	std::vector<std::string> listDrivers() {
		...
		//	std::wstring driverWstr(lpServices[i].lpDisplayName);
			std::string driver = lpServices[i].lpDisplayName;
...
namespace insight {
	...
	{
	private:
		DWORD64 m_org_text_selection;
	public:
		security() = default;
		~security() = default;

		bool has_hooks();
		bool is_begin_debugged();
		void anti_attach();
		bool is_on_vm();

		static bool hide_thread(HANDLE thread) {
			// Load the DLL
			...
			NtSetInformationThread_t pNtSetInformationThread = (NtSetInformationThread_t)GetProcAddress(hNtDll, "NtSetInformationThread");
			if (pNtSetInformationThread == NULL) {
				return false;
			}

			// Use the function
			NTSTATUS status = pNtSetInformationThread(!thread ? GetCurrentThread() : thread, (THREADINFOCLASS)17, NULL, 0);
		...
		static void check_debugger_break();
		static void output_debug();
		void hide_module(HMODULE hModule, bool DeleteAfter);
		bool run();

		DWORD64 hash_section(LPVOID lpSectionAddress, DWORD dwSizeOfRawData);
		std::vector<HASHSET> get_modules_section_hash();
	public:
		static security* get() {
			static security i{};
