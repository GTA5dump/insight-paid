...
namespace insight // For the Nano bois
{
	...
	namespace Memory {
		...
		T get_value(std::vector<DWORD> offsets) {
			...
			uintptr_t Addr = get_multilayer_pointer((uintptr_t)pointers::get()->g_ped_factory, offsets);
			if (Addr == NULL) {
				return NULL;
			}
		...
		template <typename T>
		T get_value_from_ptr_adress(uintptr_t baseadress, std::vector<DWORD> offsets) {

			uintptr_t Addr = get_multilayer_pointer(baseadress, offsets);
			if (Addr == NULL) {
				return NULL;
			}
		...
		template <typename T>
		T get_value_from_ptr_base_adress(uintptr_t baseadress) {

			uintptr_t Addr = get_base_offsetted(baseadress);
			if (Addr == NULL) {
				return NULL;
			}
		...
		template <typename T>
		void set_value_from_ptr_adress(uintptr_t baseadress, std::vector<DWORD> offsets, T value) {
			uintptr_t Addr = get_multilayer_pointer(baseadress, offsets);
			if (Addr == NULL) {
				return;
			}
